...
SimulatorはTick開始時のWorld Stateから、Robot状態、AI Runtime State、センサー情報、Robotを中心とした座標系に変換した情報を含むExecution Inputを生成する。

シミュレーターはProgramとExecution InputをAI Engineへ渡し、行動要求と更新後のAI Runtime Stateを含むExecution Resultを取得する。

シミュレーターはExecution Resultの行動要求をゲームルールに従って実行し、AI Runtime StateをWorld State内のRobotへ反映する。
