...
行動要求は移動系、切替系、攻撃系の3カテゴリに分ける。同一Tick内で同じカテゴリの行動要求が複数回生成された場合、Simulatorはそのカテゴリで最後に生成された要求だけを処理する。異なるカテゴリの要求は同一Tickで併用できる。

* 移動系：前進、後退、左旋回、右旋回、左横移動、右横移動、停止
* 切替系：武器切替
* 攻撃系：武器発射、格闘

武器発射は、同じTickの切替系要求を適用した後に選択されているWeaponだけを対象とし、複数Weaponを同時に発射しない。
