...
| code | severity | 意味 |
| --- | --- | --- |
| `invalid_program_id` | Error | Program IDが不正 |
| `missing_node_collection` | Error | Node集合が存在しない |
| `empty_program` | Error | Nodeが0件 |
| `invalid_start_node_id` | Error | `startNodeId`の形式または参照が不正 |
| `invalid_next_node_sequence` | Error | `nextNodeSequence`が不正 |
| `invalid_node_id` | Error | Node IDが不正 |
...
| `missing_instruction_id` | Error | Instruction IDが欠損 |
| `invalid_instruction_id` | Error | Instruction IDの形式が不正 |
...
| `multiple_connection_targets` | Error | 1つの出力パスに複数の接続先がある |
...
* Nodeが0件
* `startNodeId`が不正
* `nextNodeSequence`が符号付き32bit整数の有効範囲外
* `nextNodeSequence`が1未満
...
* Node IDが`node_{Program内連番}`形式ではない
* Node IDの連番が1未満または符号付き32bit整数の範囲外
...
* Instruction IDが欠損している
* Instruction IDの形式が不正
