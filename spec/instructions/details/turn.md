...
3. Robotの向きと`degree`を加算し、0以上360未満へ正規化した値を`turnTo`とする。
4. `direction`に対応する移動系行動要求を生成する。
5. `connections.next`を`nextNodeId`として解決する。
6. 行動要求をExecution Context Changesへ設定する。
7. `interruptTick`を`false`として命令実行結果を返す。

`turnTo`の計算は次のとおりとする。左旋回と右旋回で同じ計算を使用する。

```text
currentDirection = normalizeDegree(Execution InputのRobotの向き)
turnTo = normalizeDegree(currentDirection + degree)
```

`normalizeDegree`は角度を0以上360未満へ正規化する。加算前にRobotの向きを正規化し、符号付き32bit整数のオーバーフローを発生させない。
