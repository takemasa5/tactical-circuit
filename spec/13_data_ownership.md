...
Game Sessionは1回の通常シミュレーションを表す実行単位である。実行用Program、Robot設計データ、初期乱数シード、ゲームルール、World Stateを含む。
...
Simulatorは行動要求をゲームルールに従って処理し、AI Runtime StateをWorld State内のRobotへ反映する。Execution Resultは反映後に破棄する。
