...
Instructionは実行結果として選択した出力パスIDを返す。AI Engineは実行中Nodeの`connections`から対応する接続先Node IDを取得し、Program Counterを更新する。接続自体は条件を評価しない。
...
`parameterValues`はInstruction Definitionが定義するパラメータIDをキーとし、設定値を値とする。値の型として、符号付き32bit整数、真偽値、列挙値、レジスタ参照、メモリ参照、Node参照、Master Data ID参照を使用できる。実際に使用できる型は各パラメータ定義で指定する。
...
ProgramはAI実行に使用しない編集専用情報を`editorState`として、論理データと分離して保持できる。
...
作成日時と更新日時はISO 8601形式の文字列とする。Program IDと保存形式のバージョンはメタデータとは別のフィールドとして保持する。
...
パラメータや接続からNodeを参照する場合は、表示名やラベルではなくNode IDを使用する。

CALLやJUMPなどNodeを直接指定する命令は、対象を`targetNodeId`パラメータとして保持する。対象Nodeは同一Program内に存在しなければならない。
