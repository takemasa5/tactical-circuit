...
Program Editor全体はブラウザーの表示領域内に収め、ページ自体には縦横のスクロールを発生させない。内容が各領域へ収まらない場合は、ツールバー、Instructionパレット、Programキャンバス、Property Editorのうち必要な領域だけを個別にスクロールする。
...
Programキャンバスはスクロールバー、ホイール、および右ボタンドラッグで表示位置を移動できる。右ボタンドラッグでは押下位置を基準にキャンバスの縦横スクロール位置を更新し、Node位置、選択、接続、および編集履歴を変更しない。

Programキャンバス内では右ボタンドラッグ後にブラウザーのContext Menuを表示しない。右ボタン操作をNode移動、範囲選択、または接続操作として扱わない。
