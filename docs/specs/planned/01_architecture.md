...
UIからゲームロジックを直接呼び出さない。ゲームロジックからUIを呼び出さない。モジュール間の循環依存を禁止する。
...
純粋な循環プログラムとは、開始ノードから到達可能な循環のうち、条件分岐、CALL、RETURNを含まず、循環を構成するノード集合から外部へ出る接続またはNode参照を持たないものを指す。自己ループも含む。

センサー値による分岐など、条件分岐を含む循環は純粋な循環プログラムとして扱わない。

CALLまたはRETURNを含む循環はコールスタックに依存するため、純粋な循環プログラムとして扱わない。
...
- Master DataのJSON読込
- Master Dataのスキーマ検証
- Master Data IDの重複検証と参照解決
- 読み取り専用Master Dataの公開

責務外

- ゲーム進行
- 公開後のMaster Dataの更新と削除
...
新しい命令、パーツ、ロボット、武器、センサーは、既存コードの変更を最小限にして追加できる構造とする。既存の条件分岐の追加よりも、Master Dataと実装レジストリへのデータ追加で拡張できる設計を優先する。
